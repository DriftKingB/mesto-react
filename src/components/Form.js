export default function Form({ type, name, title, submitText, loadingSubmitText, submitSubline, onClose, onSubmit, isOpen, isLoading, isValid, inputFieldset }) {
  return(
    <form className={`form form_type_${type}`} name={`${name}-form`} onSubmit={onSubmit}>
      <h2 className="form__title">{title}</h2>
      {onClose && <button className="form__close-button" type="button" onClick={onClose} />}
      {inputFieldset}
